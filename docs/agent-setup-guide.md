...
> **Version:** 1.3.1 | **Java required:** 17+ | **Platforms:** Linux, Windows
...
# Windows (PowerShell)
Get-FileHash certops-agent-1.3.1.jar -Algorithm SHA256 | Select-Object Hash
```
...
# JAR is at target/certops-agent-1.3.1.jar
```
...
You can download:
- `certops-agent-1.3.1.jar` — the agent JAR (for Docker or bare-metal installation)
...
# Copy certops-agent-1.3.1.jar and Dockerfile here
```

**Step 2 — Build the image:**

```bash
docker build -t certcontrol-agent:1.3.1 .
docker tag certcontrol-agent:1.3.1 certcontrol-agent:latest
```
...
```bash
scp certops-agent-1.3.1.jar install.sh user@server:/tmp/
```
...
Copy `certops-agent-1.3.1.jar` and `install.ps1` to the same directory on the server, e.g. `C:\install\`.
...
2. Copy `certops-agent-1.3.1.jar` there
...
```
ExecStart=/usr/bin/java -Xmx512m -jar /opt/certops-agent/certops-agent-1.3.1.jar ...
```
